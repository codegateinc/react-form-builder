import { useStore, Provider } from 'outstated';
  ...
  return {
    actions: {
      ...
      setFormValue: function setFormValue(formKey, key, value, callback) {
        ...
        G.ifDefined(callback, function (fn) {
...
var useValidate = function useValidate() {
  var form = useStore(formStore);
  var config = useStore(configStore);
  return {
    ...
    validateForm: function validateForm(formName, shouldUpdateStore) {
      ...
      return form.state.formState[formName] ? G.toPairs(form.state.formState[formName]).map(function (_ref2) {
        var key = _ref2[0],
            formState = _ref2[1];
...
var useChange = function useChange() {
  var _useStore = useStore(formStore),
      state = _useStore.state,
      actions = _useStore.actions;

  var form = useStore(configStore);
...
var prepareFormInitialState = function prepareFormInitialState(formConfig, clearForm) {
  ...
  var configToPairs = G.toPairs(formConfig).map(function (_ref) {
...
var parseForm = function parseForm(state) {
  return state && G.toPairs(state).reduce(function (acc, _ref2) {
    var key = _ref2[0],
        object = _ref2[1];
...
var renderChild = function renderChild(child, formName) {
  ...
  var _useStore2 = useStore(configStore),
  ...
  if (reactElementChild.type === Input) {
    ...
    return React.cloneElement(inputChild, _extends(_extends({}, inputChild.props), {}, {
        ...
        return inputChild.props.component({
          ...
          onChangeText: function onChangeText(text) {
            return input.onChange(formName, key, text, function (state) {
              G.ifDefined(configOnUpdate[formName], function (fn) {
  ...
  if (reactElementChild.type === CheckBox) {
    ...
    return React.cloneElement(checkBoxChild, _extends(_extends({}, checkBoxChild.props), {}, {
      component: function component() {
  ...
  if (reactElementChild.type === Picker) {
    ...
    return React.cloneElement(pickerChild, _extends(_extends({}, pickerChild.props), {}, {
      component: function component() {
  ...
  if (reactElementChildren) {
    var newChildren = React.Children.map(reactElementChildren, function (child) {
      return renderChild(child, formName);
    });
...
var useForm = function useForm(_ref) {
  ...
  return {
    submitForm: function submitForm() {
      ...
      return G.ifDefined(onSuccess, function (fn) {
        return fn(parsedForm);
      });
    },
    hasChanges: state.formState[formName] && G.toPairs(state.formState[formName]).some(function (_ref2) {
      var object = _ref2[1];
    ...
    restoreToInitial: function restoreToInitial() {
      if (config.state.configStore) {
        ...
        G.ifDefined(onUpdate, function (fn) {
...
var Form = function Form(_ref) {
  ...
  return React.createElement(Fragment, null, renderForm(children, formName));
};
...
export { index as Field, Form, FormProvider, index$1 as FormTypes, useForm };
//# sourceMappingURL=react-form-builder.esm.js.map
